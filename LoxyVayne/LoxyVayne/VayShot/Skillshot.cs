    ...
    public class Skillshot
    {
        ...
        public bool IsAboutToHit(int time, Obj_AI_Base unit)
        {
            if (SpellData.Type == SkillShotType.SkillshotMissileLine)
            {
                ...
                EloBuddy.SDK.Geometry.ProjectionInfo projection = unit.ServerPosition.To2D().ProjectOn(missilePos, missilePosAfterT);
        ...
        public bool IsAboutToHit(int time, Vector3 position)
        {
            if (SpellData.Type == SkillShotType.SkillshotMissileLine)
            {
                Vector2 missilePos = GetMissilePosition(0);
                Vector2 missilePosAfterT = GetMissilePosition(time);
                EloBuddy.SDK.Geometry.ProjectionInfo projection = position.To2D().ProjectOn(missilePos, missilePosAfterT);
